  ...
  $.fn.srolling = function (settings) {
    ...
    function _start_box() {
      switch (move) {
        case 'down':
        case 'top':
          box_area.css('top', start_left);
          break;
        case 'right':
        case 'left':
          box_area.css('left', start_left);
          break;
      }
    }
      ...
      switch (move) {
        ...
        case 'top':
          ...
          box_area.animate({
            ...
            for (var i = 0; i < item_count; i++) {
              box_area.children().first().remove();
            }
            _create_next();
            _start_box();
            prev_obj.click(_prev_act);
    ...
    function _items() {
      ...
      for (var i = cache_count; i > 0; i--) {
        if (item_total <= prev_idx || prev_idx < 0) {
          prev_idx = item_total - 1;
        }
      ...
      for (var i = 0; i < cache_count; i++) {
        if (item_total <= next_idx || next_idx < 0) {
          next_idx = 0;
        }
        var t_obj = item_area.clone();
